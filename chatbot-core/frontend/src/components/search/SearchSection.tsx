...
import { cancellable, CancellationToken } from '@/lib/search/cancellable';
...
export type searchState = 'input' | 'searching' | 'reading' | 'analyzing' | 'summarizing' | 'generating' | 'citing';
...
export const SearchSection = ({ toggle, toggledSidebar, defaultSearchType }: SearchSectionProps) => {
  const {
    querySessions,
    ccPairs,
    documentSets,
  ...
  const toggleAgentic = useCallback(() => {
    Cookies.set(AGENTIC_SEARCH_TYPE_COOKIE_NAME, String(!agentic).toLocaleLowerCase());
  ...
  const selectedPersona = assistants.find(assistant => assistant.id === 0) ? 0 : assistants[0]?.id;
  ...
  const [finalAvailableSources, finalAvailableDocumentSets] = computeAvailableFilters({
    selectedPersona: assistants.find(assistant => assistant.id === selectedPersona),
  ...
  useEffect(() => {
    ...
    function extractFirstMessageByType(chatSession: SearchSession, messageType: 'user' | 'assistant'): string | null {
      const userMessage = chatSession?.messages.find(msg => msg.message_type === messageType);
      return userMessage ? userMessage.message : null;
    }

    async function initialSessionFetch() {
      const response = await fetch(`/api/query/search-session/${existingSearchessionId}`);
      const searchSession = (await response.json()) as SearchSession;
      const userMessage = extractFirstMessageByType(searchSession, 'user');
      const assistantMessage = extractFirstMessageByType(searchSession, 'assistant');
  ...
  const [defaultOverrides, setDefaultOverrides] = useState<SearchDefaultOverrides>(SEARCH_DEFAULT_OVERRIDES_START);

  const newSearchState = (currentSearchState: searchState, newSearchState: searchState) => {
  ...
  const updateCurrentAnswer = (answer: string) => {
    ...
    if (analyzeStartTime) {
      ...
      setTimeout(() => {
        setSearchState(searchState => newSearchState(searchState, 'generating'));
  ...
  const updateMessageAndThreadId = (messageId: number, chat_session_id: string) => {
    setSearchResponse(prevState => ({
      ...(prevState || initialSearchResponse),
      messageId,
    }));
  ...
  const [previousSearch, setPreviousSearch] = useState<null | SearchDetails>(null);
  ...
  const currentSearch = (overrideMessage?: string): SearchDetails => {
    return {
      ...
      persona: assistants.find(assistant => assistant.id === selectedPersona) as Persona,
    };
  };
  ...
  const onSearch = async ({ searchType, agentic, offset, overrideMessage }: SearchRequestOverrides = {}) => {
  ...
  const shouldUseAgenticDisplay =
    ...
    (searchResponse.documents || []).some(
      document =>
        searchResponse.additional_relevance && searchResponse.additional_relevance[document.document_id] !== undefined,
    );
  ...
  return (
    <>
      <div className="flex relative pr-[8px] h-full text-default">
        ...
        {!shouldDisplayNoSources && showApiKeyModal && !shouldShowWelcomeModal && (
          <ApiKeyModal setPopup={setPopup} hide={() => setShowApiKeyModal(false)} />
        )}
                    ...
                    >
                      <div className="mt-48 mb-8 flex justify-center items-center">
                        <div className="w-message-xs 2xl:w-message-sm 3xl:w-message">
                          <div className="flex">
                            <div className="text-3xl font-bold font-strong text-strong mx-auto">Unlock Knowledge</div>
                          </div>
                        </div>
                      </div>
                    </div>
