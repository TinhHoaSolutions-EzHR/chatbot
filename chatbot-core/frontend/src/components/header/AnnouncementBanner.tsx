...
export function AnnouncementBanner() {
  ...
  const [localNotifications, setLocalNotifications] = useState(settings?.settings.notifications || []);

  useEffect(() => {
    const filteredNotifications = (settings?.settings.notifications || []).filter(
      notification => !Cookies.get(`${DISMISSED_NOTIFICATION_COOKIE_PREFIX}${notification.id}`),
    );
  ...
  const handleDismiss = async (notificationId: number) => {
    try {
      ...
      if (response.ok) {
        Cookies.set(`${DISMISSED_NOTIFICATION_COOKIE_PREFIX}${notificationId}`, 'true', {
            ...
            >
              {notification.notif_type == 'reindex' ? (
                <p className="text-center">
                  ...
                  <Link href={'/admin/configuration/search'} className="ml-2 underline cursor-pointer">
                    Update here
                  </Link>
                </p>
              ) : null}
