...
import { AnswerPiecePacket, DanswerDocument, StreamStopInfo, StreamStopReason } from '@/lib/search/interfaces';
...
export function ChatPage({
  ...
  const [sendOnLoad, setSendOnLoad] = useState<string | null>(searchParams.get(SEARCH_PARAM_NAMES.SEND_ON_LOAD));

  const currentPersonaId = searchParams.get(SEARCH_PARAM_NAMES.PERSONA_ID);
  const modelVersionFromSearchParams = searchParams.get(SEARCH_PARAM_NAMES.STRUCTURED_MODEL);
  ...
  const selectedChatSession = chatSessions.find(chatSession => chatSession.id === existingChatSessionId);
  ...
  const [selectedAssistant, setSelectedAssistant] = useState<Persona | undefined>(
    ...
    existingChatSessionAssistantId !== undefined
      ? availableAssistants.find(assistant => assistant.id === existingChatSessionAssistantId)
      : defaultAssistantId !== undefined
      ? availableAssistants.find(assistant => assistant.id === defaultAssistantId)
      : undefined,
  );
  // Gather default temperature settings
  const search_param_temperature = searchParams.get(SEARCH_PARAM_NAMES.TEMPERATURE);

  const defaultTemperature = search_param_temperature
    ? parseFloat(search_param_temperature)
    : selectedAssistant?.tools.some(
        tool => tool.in_code_tool_id === 'SearchTool' || tool.in_code_tool_id === 'InternetSearchTool',
      )
    ? 0
    : 0.7;
  ...
  const setSelectedAssistantFromId = (assistantId: number) => {
    ...
    // for old chats
    setSelectedAssistant(availableAssistants.find(assistant => assistant.id === assistantId));
  };
  ...
  const [alternativeAssistant, setAlternativeAssistant] = useState<Persona | null>(null);
  ...
  const liveAssistant =
    alternativeAssistant || selectedAssistant || recentAssistants[0] || finalAssistants[0] || availableAssistants[0];

  const noAssistants = liveAssistant == null || liveAssistant == undefined;
  // always set the model override for the chat session, when an assistant, llm provider, or user preference exists
  useEffect(() => {
    const personaDefault = getLLMProviderOverrideForPersona(liveAssistant, llmProviders);

    if (personaDefault) {
      llmOverrideManager.setLlmOverride(personaDefault);
    } else if (user?.preferences.default_model) {
      llmOverrideManager.setLlmOverride(destructureValue(user?.preferences.default_model));
    }
  ...
  const [alternativeGeneratingAssistant, setAlternativeGeneratingAssistant] = useState<Persona | null>(null);
  ...
  const [isFetchingChatMessages, setIsFetchingChatMessages] = useState(existingChatSessionId !== null);
  ...
  useEffect(() => {
    ...
    const isChatSessionSwitch = chatSessionIdRef.current !== null && existingChatSessionId !== priorChatSessionId;
    ...
    async function initialSessionFetch() {
      ...
      const shouldScrollToBottom =
        visibleRange.get(existingChatSessionId) === undefined || visibleRange.get(existingChatSessionId)?.end == 0;

      clearSelectedDocuments();
      setIsFetchingChatMessages(true);
      const response = await fetch(`/api/chat/get-chat-session/${existingChatSessionId}`);
      ...
      // stream
      if (
        (messageHistory[messageHistory.length - 1]?.type !== 'error' || loadedSessionId != null) &&
  ...
  const [message, setMessage] = useState(searchParams.get(SEARCH_PARAM_NAMES.USER_PROMPT) || '');

  const [completeMessageDetail, setCompleteMessageDetail] = useState<Map<string | null, Map<number, Message>>>(
    new Map(),
  );

  const updateCompleteMessageDetail = (sessionId: string | null, messageMap: Map<number, Message>) => {
  ...
  const currentMessageMap = (messageDetail: Map<string | null, Map<number, Message>>) => {
    return messageDetail.get(chatSessionIdRef.current) || new Map<number, Message>();
  };
  ...
  const upsertToCompleteMessageMap = ({
    ...
    // deep copy
    const frozenCompleteMessageMap = completeMessageMapOverride || currentMessageMap(completeMessageDetail);
    ...
    if (makeLatestChildMessage) {
      ...
      if (latestMessage) {
        newCompleteMessageMap.get(latestMessage.messageId)!.latestChildMessageId = messages[0].messageId;
      }
    }
    ...
    updateCompleteMessageDetail(chatSessionId || currentSessionId(), newCompleteMessageMap);
    return newCompleteMessageDetail;
  };

  const messageHistory = buildLatestMessageChain(currentMessageMap(completeMessageDetail));
  ...
  const [regenerationState, setRegenerationState] = useState<Map<string | null, RegenerationState | null>>(
    new Map([[null, null]]),
  );

  const [abortControllers, setAbortControllers] = useState<Map<string | null, AbortController>>(new Map());
  ...
  const updateStatesWithNewSessionId = (newSessionId: string) => {
    const updateState = (setState: Dispatch<SetStateAction<Map<string | null, any>>>, defaultValue?: any) => {
      setState(prevState => {
        const newState = new Map(prevState);
  ...
  const updateChatState = (newState: ChatState, sessionId?: string | null) => {
    setChatState(prevState => {
      ...
      newChatState.set(sessionId !== undefined ? sessionId : currentSessionId(), newState);
      return newChatState;
    });
  };
  ...
  const currentChatAnswering = () => {
    return currentChatState() == 'toolBuilding' || currentChatState() == 'streaming' || currentChatState() == 'loading';
  };

  const updateRegenerationState = (newState: RegenerationState | null, sessionId?: string | null) => {
    setRegenerationState(prevState => {
      const newRegenerationState = new Map(prevState);
      newRegenerationState.set(sessionId !== undefined ? sessionId : currentSessionId(), newState);
  ...
  const [canContinue, setCanContinue] = useState<Map<string | null, boolean>>(new Map([[null, false]]));
  ...
  const updateCanContinue = (newState: boolean, sessionId?: string | null) => {
    setCanContinue(prevState => {
      ...
      newCanContinueState.set(sessionId !== undefined ? sessionId : currentSessionId(), newState);
  ...
  const { aiMessage } = selectedMessageForDocDisplay
    ? getHumanAndAIMessageFromMessageNumber(messageHistory, selectedMessageForDocDisplay)
    : { aiMessage: null };
  ...
  useEffect(() => {
    if (messageHistory.length === 0 && chatSessionIdRef.current === null) {
      ...
      setSelectedAssistant(availableAssistants.find(persona => persona.id === defaultAssistantId));
    }
  }, [defaultAssistantId, availableAssistants, messageHistory.length]);

  const [selectedDocuments, toggleDocumentSelection, clearSelectedDocuments, selectedDocumentTokens] =
  ...
  useEffect(() => {
    async function fetchMaxTokens() {
      const response = await fetch(`/api/chat/max-selected-document-tokens?persona_id=${liveAssistant.id}`);
      if (response.ok) {
  ...
  const previousHeight = useRef<number>(inputRef.current?.getBoundingClientRect().height!);
  const scrollDist = useRef<number>(0);

  const updateScrollTracking = () => {
    const scrollDistance =
      endDivRef?.current?.getBoundingClientRect()?.top! - inputRef?.current?.getBoundingClientRect()?.top!;
  ...
  const handleInputResize = () => {
    setTimeout(() => {
      if (inputRef.current && lastMessageRef.current && !waitForScrollRef.current) {
        const newHeight: number = inputRef.current?.getBoundingClientRect().height!;
    ...
    setTimeout(() => {
      ...
      if (currentVisibleRange.end < messageHistory.length) {
        ...
        updateCurrentVisibleRange({
          start: Math.max(0, messageHistory.length - (currentVisibleRange.end - currentVisibleRange.start)),
  ...
  const [hasPerformedInitialScroll, setHasPerformedInitialScroll] = useState(existingChatSessionId === null);
  ...
  async function updateCurrentMessageFIFO(stack: CurrentMessageFIFO, params: any) {
    try {
  ...
  const onSubmit = async ({
    ...
    if (isNewSession) {
      currChatSessionId = await createChatSession(liveAssistant?.id || 0, searchParamBasedChatSessionName);
    } else {
    ...
    setAbortControllers(prev => new Map(prev).set(currChatSessionId, controller));

    const messageToResend = messageHistory.find(message => message.messageId === messageIdToResend);

    updateRegenerationState(
      regenerationRequest ? { regenerating: true, finalMessageIndex: messageIdToResend || 0 } : null,
    );
    const messageMap = currentMessageMap(completeMessageDetail);
    const messageToResendParent =
      messageToResend?.parentMessageId !== null && messageToResend?.parentMessageId !== undefined
    ...
    if (!messageToResend && messageIdToResend !== undefined) {
      setPopup({
        message: 'Failed to re-send message - please refresh the page and try again.',
        type: 'error',
      });
    ...
    const currMessageHistory =
      messageToResendIndex !== null ? messageHistory.slice(0, messageToResendIndex) : messageHistory;

    let parentMessage =
      messageToResendParent ||
      (currMessageHistory.length > 0 ? currMessageHistory[currMessageHistory.length - 1] : null) ||
    ...
    let retrievalType: RetrievalType = selectedDocuments.length > 0 ? RetrievalType.SelectedDocs : RetrievalType.None;
    ...
    try {
      const mapKeys = Array.from(currentMessageMap(completeMessageDetail).keys());
      const systemMessage = Math.min(...mapKeys);

      const lastSuccessfulMessageId = getLastSuccessfulMessageId(currMessageHistory) || systemMessage;
      ...
      updateCurrentMessageFIFO(stack, {
        ...
        parentMessageId: regenerationRequest?.parentMessage.messageId || lastSuccessfulMessageId,
        ...
        systemPromptOverride: searchParams.get(SEARCH_PARAM_NAMES.SYSTEM_PROMPT) || undefined,
      ...
      while (!stack.isComplete || !stack.isEmpty()) {
        ...
        if (!stack.isEmpty() && !controller.signal.aborted) {
          ...
          if (!initialFetchDetails) {
            ...
            const assistant_message_id = messageResponseIDInfo.reserved_assistant_message_id;

            // we will use tempMessages until the regenerated message is complete
            messageUpdates = [
              {
                messageId: regenerationRequest ? regenerationRequest?.parentMessage?.messageId! : user_message_id,
                message: currMessage,
                type: 'user',
            ...
            if (parentMessage && !regenerationRequest) {
              messageUpdates.push({
                ...parentMessage,
                childrenMessageIds: (parentMessage.childrenMessageIds || []).concat([user_message_id]),
                latestChildMessageId: user_message_id,
              });
            }

            const { messageMap: currentFrozenMessageMap } = upsertToCompleteMessageMap({
            ...
            setChatState(prevState => {
              if (prevState.get(chatSessionIdRef.current!) === 'loading') {
                return new Map(prevState).set(chatSessionIdRef.current!, 'streaming');
              }
              return prevState;
            });
            ...
            if (Object.hasOwn(packet, 'answer_piece')) {
              ...
              aiMessageImages = (packet as FileChatDisplay).file_ids.map(fileId => {
                return {
                  id: fileId,
            ...
            parentMessage = parentMessage || frozenMessageMap?.get(SYSTEM_MESSAGE_ID)!;
            ...
            const updateFn = (messages: Message[]) => {
              const replacementsMap = regenerationRequest
                ? new Map([
                    ...
                    [regenerationRequest?.messageId, initialFetchDetails?.assistant_message_id],
                  ] as [number, number][])
                : null;
      ...
      upsertToCompleteMessageMap({
        messages: [
          {
            messageId: initialFetchDetails?.user_message_id || TEMP_USER_MESSAGE_ID,
            message: currMessage,
            type: 'user',
            files: currentMessageFiles,
            toolCall: null,
          ...
          {
            messageId: initialFetchDetails?.assistant_message_id || TEMP_ASSISTANT_MESSAGE_ID,
            message: errorMsg,
            type: 'error',
            files: aiMessageImages || [],
            toolCall: null,
            parentMessageId: initialFetchDetails?.user_message_id || TEMP_USER_MESSAGE_ID,
          },
        ],
    ...
    if (isNewSession) {
      ...
      if (currChatSessionId === chatSessionIdRef.current || chatSessionIdRef.current === null) {
  ...
  const onFeedback = async (
    ...
    const response = await handleChatFeedback(messageId, feedbackType, feedbackDetails, predefinedFeedback);
  ...
  const handleImageUpload = (acceptedFiles: File[]) => {
    const [_, llmModel] = getFinalLLM(llmProviders, liveAssistant, llmOverrideManager.llmOverride);
    ...
    if (imageFiles.length > 0 && !llmAcceptsImages) {
      setPopup({
        type: 'error',
        message: 'The current Assistant does not support image input. Please select an assistant with Vision support.',
      });
      return;
    }
    ...
    const tempFileDescriptors = acceptedFiles.map(file => ({
      id: uuidv4(),
      type: file.type.startsWith('image/') ? ChatFileType.IMAGE : ChatFileType.DOCUMENT,
      isUploading: true,
    }));
    ...
    const removeTempFiles = (prev: FileDescriptor[]) => {
      return prev.filter(file => !tempFileDescriptors.some(newFile => newFile.id === file.id));
    };
  ...
  const toggleSidebar = () => {
    Cookies.set(SIDEBAR_TOGGLED_COOKIE_NAME, String(!toggledSidebar).toLocaleLowerCase()),
      {
        path: '/',
      };
  ...
  const [visibleRange, setVisibleRange] = useState<Map<string | null, VisibleRange>>(() => {
  ...
  const updateCurrentVisibleRange = (newRange: VisibleRange, forceUpdate?: boolean) => {
    if (scrollInitialized.current && visibleRange.get(loadedIdSessionRef.current) == undefined && !forceUpdate) {
      return;
    }
  ...
  const initializeVisibleRange = () => {
    ...
    if (!scrollInitialized.current && upToDatemessageHistory.length > 0) {
      ...
      const newMostVisibleMessageId = upToDatemessageHistory[newEnd - 1]?.messageId;
  ...
  const updateVisibleRangeBasedOnScroll = () => {
    ...
    if (mostVisibleMessageIndex !== -1) {
      ...
      const endIndex = Math.min(messageHistory.length, mostVisibleMessageIndex + BUFFER_COUNT + 1);
  ...
  useEffect(() => {
    ...
    const includes = checkAnyAssistantHasSearch(messageHistory, availableAssistants, liveAssistant);
  ...
  const [retrievalEnabled, setRetrievalEnabled] = useState(() => {
    ...
    return checkAnyAssistantHasSearch(messageHistory, availableAssistants, liveAssistant);
  });
  const [stackTraceModalContent, setStackTraceModalContent] = useState<string | null>(null);
  ...
  const [sharedChatSession, setSharedChatSession] = useState<ChatSession | null>();
  const [deletingChatSession, setDeletingChatSession] = useState<ChatSession | null>();
  ...
  return (
    <>
      ...
      {showApiKeyModal && !shouldShowWelcomeModal ? (
        <ApiKeyModal hide={() => setShowApiKeyModal(false)} setPopup={setPopup} />
      ) : (
      ...
      {currentFeedback && (
        <FeedbackModal
          ...
          onSubmit={({ message, predefinedFeedback }) => {
            onFeedback(currentFeedback[1], currentFeedback[0], message, predefinedFeedback);
      ...
      {sharedChatSession && (
        <ShareChatSessionModal
          ...
          onShare={shared =>
            setChatSessionSharedStatus(shared ? ChatSessionSharedStatus.Public : ChatSessionSharedStatus.Private)
          }
        />
      )}
      ...
      <div className="fixed inset-0 flex flex-col text-default">
        <div className="h-[100dvh] overflow-y-hidden">
          ...
          <BlurBackground visible={!untoggled && (showDocSidebar || toggledSidebar)} />

          <div ref={masterFlexboxRef} className="flex h-full w-full overflow-x-hidden">
            <div className="flex h-full flex-col w-full">
              {liveAssistant && (
                <FunctionalHeader
                  ...
                  reset={() => setMessage('')}
                  page="chat"
                  setSharingModalVisible={chatSessionIdRef.current !== null ? setSharingModalVisible : undefined}
                  toggleSidebar={toggleSidebar}
                  currentChatSession={selectedChatSession}
                />
              )}

              {documentSidebarInitialWidth !== undefined && isReady && !isLoadingUser ? (
                      ...
                      the top of the chat page. Oly used in the EE version of the app. */}
                                ...
                                {!isFetchingChatMessages &&
                                  ...
                                  allAssistants.length > 1 && (
                                    <div className="mx-auto px-4 w-full max-w-[750px] flex flex-col items-center">
                                      ...
                                      <div className="text-sm text-black font-medium mb-4">Recent Assistants</div>
                                      <AssistantBanner
                          ...
                          <div
                            className={
                              ...
                              (settings?.enterpriseSettings?.two_lines_for_chat_header ? 'mt-20 ' : 'mt-8') +
                              (hasPerformedInitialScroll ? '' : 'invisible')
                            }
                          >
                            {(messageHistory.length < BUFFER_COUNT
                              ? messageHistory
                              : messageHistory.slice(currentVisibleRange.start, currentVisibleRange.end)
                              ...
                              if (message.type === 'user') {
                                if (
                                  (currentSessionChatState == 'loading' && i == messageHistory.length - 1) ||
                                  (currentSessionRegenerationState?.regenerating &&
                                    message.messageId >= currentSessionRegenerationState?.finalMessageIndex!)
                                ) {
                                  return <></>;
                                }
                                return (
                                  <div id={`message-${message.messageId}`} key={messageReactComponentKey}>
                                    <HumanMessage
                                      stopGenerating={stopGenerating}
                                      ...
                                      onMessageSelection={messageId => {
                                        ...
                                        newCompleteMessageMap.get(message.parentMessageId!)!.latestChildMessageId =
                                          messageId;
                                        updateCompleteMessageDetail(currentSessionId(), newCompleteMessageMap);
                                ...
                                const currentAlternativeAssistant =
                                  message.alternateAssistantID != null
                                    ? availableAssistants.find(persona => persona.id == message.alternateAssistantID)
                                    : null;
                                ...
                                return (
                                  ...
                                  >
                                    <AIMessage
                                      ...
                                      handleFeedback={
                                        i === messageHistory.length - 1 && currentSessionChatState != 'input'
                                          ? undefined
                                          : feedbackType =>
                                              setCurrentFeedback([feedbackType, message.messageId as number])
                                      }
                                      ...
                                      handleForceSearch={() => {
                                        if (previousMessage && previousMessage.messageId) {
                                          ...
                                          setPopup({
                                            type: 'error',
                                            message: 'Failed to force search - please refresh the page and try again.',
                                          });
                                        }
                                      }}
                                      retrievalDisabled={
                            ...
                            {(currentSessionChatState == 'loading' ||
                              ...
                              <HumanMessage key={-2} messageId={-1} content={submittedMessage} />
                            )}
                            ...
                            {loadingError && (
                              <div key={-1}>
                                <AIMessage
                                  currentPersona={liveAssistant}
                                  messageId={-1}
                                  content={<p className="text-red-700 text-sm my-auto">{loadingError}</p>}
                                />
                              </div>
                            )}
                        ...
                        <div ref={inputRef} className="absolute bottom-0 z-10 w-full">
                          <div className="w-[95%] mx-auto relative mb-8">
                            ...
                            <ChatInputBar
                              ...
                              chatSessionId={chatSessionIdRef.current!}
                            />
                            {enterpriseSettings && enterpriseSettings.custom_lower_disclaimer_content && (
                              <div className="mobile:hidden mt-4 flex items-center justify-center relative w-[95%] mx-auto">
                                <div className="text-sm text-text-500 max-w-searchbar-max px-4 text-center">
                                  <MinimalMarkdown content={enterpriseSettings.custom_lower_disclaimer_content} />
                                </div>
                              </div>
                            )}
