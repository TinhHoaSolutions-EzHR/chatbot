    ...
    >
      <div className="w-full">
        <div className="flex items-center mb-2">
          ...
          <div className="ml-2 ellipsis truncate font-bold text-sm text-emphasis">{assistant.name}</div>
        </div>

        <div className="text-xs text-wrap text-subtle mb-2 mt-2 line-clamp-3 py-1">{assistant.description}</div>
        <div className="mt-2 flex flex-col gap-y-1">
          ...
          <div className="text-xs text-subtle">
            ...
            {getDisplayNameForModel(assistant.llm_model_version_override || llmName)}
          </div>
...
export function DraggableAssistantCard(props: {
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: props.assistant.id.toString(),
  });
  ...
  return (
    <div ref={setNodeRef} style={style} className="overlow-y-scroll inputscroll flex items-center">
